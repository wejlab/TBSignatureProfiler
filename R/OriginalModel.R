...
#'   Default is NULL. If geneSignaturesName is NULL, 
#'   run all available gene signatures' original model in the package.
#' @param BPPARAM An instance inherited from \code{bplappy}.
#'   See \code{\link[BiocParallel]{bplapply}} for details.

#' @return A SummarizedExperiment object with predicted scores for each sample obtained from the signature's original model.
#' @export
#' @examples
...
evaluateOriginalModel <- function(input, useAssay = 1, geneSignaturesName = NULL,
    ...
    # If signatures is default, run all the original model for all avaiable gene signatures
    if(is.null(geneSignaturesName)) { geneSignaturesName <- signature_all }
    ...
    else {sample_score_NoRetraining <- NULL}
    ...
    else {sample_score_Retraining <- NULL}
...
#' 
#' A function to obtain predicted score for TB gene signatures that required 
#' retraining of their original model.
#' 
            ...
            if (sig == "Leong_RISK_29"){
                ...
                if (requireNamespace("dplyr", quietly = TRUE)) {
                  sample_baseline_GSE79362 <- col_info[-index_NA, c("PatientID","MeasurementTime")] %>%
                    data.frame() %>% dplyr::mutate(sample_name=row.names(col_info)[-index_NA]) %>%
...
subsetGeneSet <- function(theObject, gene_set, useAssay = 1,
    ...
    if(obtainDiagnosis == FALSE) { return(dat_assay_sig) }
    else {
...
ref_combat_impute <- function(theObject_train, useAssay = 1, gene_set, input,
    ...
    ############### Perform Reference Batch Correction #################
    # Keep the training data as always the same
    ...
    # Make list that contains only signature gene sets
    dat_exprs_match_sub <- list(GSE_train = data.frame(dat_train_assay_sig),
                                GSE_test = data.frame(dat_test_assay_sig))
    
    # Merge training and testing based on gene features in the training data
    dat_exprs_combine <- Reduce(
        function(x, y) merge(x, y, all.x = TRUE, by = "id"),
        lapply(dat_exprs_match_sub, function(x) { x$id <- rownames(x); x }))
